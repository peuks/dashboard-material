...
const validationSchema = yup.object({
    ...
    size: yup.number('Enter your size').required('Size selection is required.')
});
        ...
        <Grid item>
            <Tooltip title={colorsData[0].label}>
                <ButtonBase sx={{ borderRadius: '50%' }}>
                    ...
                    >
                        {checked && (
                            <CircleIcon color={theme.palette.mode === 'light' ? 'grey.50' : 'grey.800'} sx={{ fontSize: '0.75rem' }} />
                        )}
                        {!checked && <CircleIcon color={colorsData[0].bg} sx={{ fontSize: '0.75rem', color: colorsData[0].bg }} />}
                    </Avatar>
                </ButtonBase>
            </Tooltip>
        </Grid>
...
const Increment = (props) => {
    ...
    return (
        <ButtonGroup size="large" variant="string" sx={{ border: '1px solid', borderColor: 'grey.400' }}>
            <Button
                key="three"
                disabled={value <= 1}
